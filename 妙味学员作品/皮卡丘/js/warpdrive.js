!function(){
    function a(a,b){
        function da(){
            W=document.createElement("canvas"),//创建canvas
                W.style.backgroundColor=ga(q.r,q.g,q.b),//颜色
                W.setAttribute("width",g),//设置元素的宽
                W.setAttribute("height",h),//设置元素的高
            c.addMouseControls&&(W.addEventListener("mousemove",ta,!1),//给指定元素添加事件
            ...
            c.addTouchControls&&(W.addEventListener("touchstart",za,!1),//触碰控制
                W.addEventListener("touchend",Aa,!1),
                W.addEventListener("touchmove",Ba,!1),
                W.addEventListener("touchcancel",Ca,!1)),
            c.hideContextMenu&&(W.oncontextmenu=function(a){a.preventDefault()}),
                a.appendChild(W),X=W.getContext("2d"),
                Y=X.getImageData(0,0,g,h),Z=Y.data,$={x:W.width/2,y:W.height/2},_=!1,aa=!1,ba={x:$.x,y:$.y},
                U=[],V=[],S=[],T=[],la(U,o,p,q,n),la(V,l,m,q,k),oa(),pa(),
            ...
            var d=a<0?-1*a:a,
                ...
                f=parseInt,
                g=f(b.slice(1),16),
                h=f((c||(a<0?"#000000":"#FFFFFF")).slice(1),16),i=g>>16,j=g>>8&255,
                k=255&g;
            return"#"+(16777216+65536*(e(((h>>16)-i)*d)+i)+256*(e(((h>>8&255)-j)*d)+j)+(e(((255&h)-k)*d)+k)).toString(16).slice(1)
        }
            function fa(a){
                var b=/^#?([a-f\d])([a-f\d])([a-f\d])$/i;a=a.replace(b,function(a,b,c,d){return b+b+c+c+d+d});
                var c=/^#?([a-f\d]{2})([a-f\d]{2})([a-f\d]{2})$/i.exec(a);return c?{r:parseInt(c[1],16),
                    g:parseInt(c[2],16),b:parseInt(c[3],16)}:null}
                    function ga(a,b,c){
                ...
                function ha(a){
                    ...
                    function ka(a,b,c,d,e,f,i,j){
                        for(var k=Math.abs(c-a),l=Math.abs(d-b),m=a<c?1:-1,n=b<d?1:-1,o=k-l,p=a,q=b;;){
                            if(p>-1&&p<g&&q>-1&&q<h&&ja(p,q,e,f,i,j),p===c&&q===d)
                                break;
                            var r=2*o;r>-k&&(o-=l,p+=m),r<l&&(o+=k,q+=n)
                        }
                    }
                    function la(a,b,c,d,e){
                        var j,k,l,m,n,f=ga(d.r,d.g,d.b),g=ga(e.r,e.g,e.b),h=[],i=[];
                        for(k=0,l=100;k<=l;k++)j=k/100,h.push(ea(j,f,g));
                        for(k=0,l=c-b;k<=l;k++){
                            var o=k+b;i.push(h[o]
                            )
                        }
                        for(k=0,l=i.length;k<l;k++){
                            ...
                            function ma(a,b){
                        var c=Math.random()*b-b/2,d=Math.random()*b-b/2;if(a>0)
                        ...
                        function na(a,b,c,d,e,f){
                            ...
                            var a,b,c,d,e,f;
                            for(a=0;a<j;a++)
                                f=ma(0,2e4),b=f.x,c=f.y,d=Math.round(Math.random()*P),
                                    e=na(b,c,d,b,c,d),
                            ...
                            for(a=0;a<i;a++)f=ma(L,1e4),b=f.x,c=f.y,d=Math.round(Math.random()*P),
                                e=na(b,c,d,b,c,d),e.distance=P-d,e.distanceTotal=Math.round(P+x),
                                e.colorIndex=Math.floor(Math.random()*U.length),
                                e.colorLookupTable=U[e.colorIndex],
                                e.color=e.colorLookupTable[Math.floor(e.distance/e.distanceTotal*100)],S.push(e)
                        }
                                function pa(){
                                requestAnimFrame(pa),ca||qa()}function qa(){ia();
                                var a,c,j,k;D&&(aa?(J+=1)>I&&(J=I):(J-=1)<0&&(J=0),
                F=Math.easeOutQuad(J,G,H,I),C-=F*E),_?(w+=1,w>v&&(w=v),B-=1,B<0&&(B=0),
            N&&($.x+=(ba.x-$.x)*M),O&&($.y+=(ba.y-$.y)*M)):(w-=1,w<0&&(w=0),B+=1,B>A&&(B=A),
            N&&($.x+=(W.width/2-$.x)*M),O&&($.y+=(W.height/2-$.y)*M)),
                s=Math.easeOutQuad(w,0,u-t,v)+t,x=Math.easeInQuad(B,0,z-y,A)+y,Q=1-x;
                                var l=s*(s/(u/K));
                                for(a=0,c=T.length;a<c;a++)
                                    j=T[a],k=x/(x+j.z),j.x2d=j.x*k+$.x,j.y2d=j.y*k+$.y,j.x2d>-1&&j.x2d<g&&j.y2d>-1&&j.y2d<h&&ja(0|j.x2d,0|j.y2d,j.color.r,j.color.g,j.color.b,255);
                                for(a=0,c=S.length;a<c;a++){
                                    if(j=S[a],j.distanceTotal=Math.round(P+x),r>=0?(j.z-=s,j.distance+=s,j.z<Q&&(j.z=R,j.distance=0)):(j.z+=s,j.distance-=s,j.z>R&&(j.z=Q,j.distance=j.distanceTotal)),j.color=j.colorLookupTable[Math.floor(j.distance/j.distanceTotal*100)],k=x/(x+j.z),j.x2d=j.x*k+$.x,j.y2d=j.y*k+$.y,s===t)j.x2d>-1&&j.x2d<g&&j.y2d>-1&&j.y2d<h&&ja(0|j.x2d,0|j.y2d,j.color.r,j.color.g,j.color.b,255);
                                        ...
                                        var m=j.z+l;k=x/(x+m);
                                        var n=j.x*k+$.x,o=j.y*k+$.y;n>-1&&n<g&&o>-1&&o<h&&ka(0|j.x2d,0|j.y2d,0|n,0|o,j.color.r,j.color.g,j.color.b,255)}
                                        if(F!==G){
                                        var p=e*C,q=Math.cos(p),L=Math.sin(p);
                                        j.x=q*j.ox+L*j.oy,j.y=q*j.oy-L*j.ox}}X.putImageData(Y,0,0)}
                                        function ra(a){
                                          sa()
                                }
                                    function sa(){
